...
export class FindingsController {
  ...
  @Post()
  @ApiOperation({ summary: 'Crear hallazgo', description: 'Registra un nuevo hallazgo de auditoría vinculado a un control HIPAA. Solo ADMIN y AUDITOR.' })
  ...
  @Get()
  @ApiOperation({ summary: 'Listar hallazgos', description: 'Retorna todos los hallazgos de auditoría ordenados por fecha de creación descendente.' })
  @ApiResponse({ status: 200, description: 'Lista de hallazgos con usuario asignado' })
  findAll() {
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar hallazgo', description: 'Modifica estado, severidad, descripción, responsable o fecha límite. Cuando el estado cambia a Closed o Remediated se registra la fecha de cierre automáticamente.' })
  @ApiParam({ name: 'id', type: Number, description: 'ID del hallazgo a modificar' })
  ...
  @ApiParam({ name: 'id', type: Number, description: 'ID del hallazgo a eliminar' })
